...
- **Product access (rewritten 2026-07-24):** everyone may create a Free account and save onboarding. Only Pro, Agentic, or a current beta grant may execute the deliverable, receive daily briefs, or use integrations. **Beta access is now database-backed and approval-only** — a signed-in account **applies** (`/four` → beta card → `POST /api/beta`), which grants nothing, and **Avi approves by hand** at **`/admin.html`**. Approval starts a window ending after **7 days or 7 full reports, whichever comes first**; then the account reverts to Free and must buy Pro. **The `GK_BETA_EMAILS` allowlist and the `GK_BETA_OPEN` flag were deleted** — there is no second door. Admin rights come from `GK_ADMIN_USER_IDS` (Supabase **user ids**, never emails) and fail closed. A paid subscription is always checked first, so a customer can never be locked out by beta expiry. `GK_BETA_ENABLED=0` still halts all beta instantly; `GK_BETA_EXPIRES_AT` still ends every grant at once. **Full detail + the two required deployment steps: `docs/beta.md`.** If paid/beta access ends, completed reports remain readable, but generating new ones locks. **Product model (rewritten 2026-07-25, Phase 2):** every eligible account generates **one full report per UTC day, on any company** — no company lock — and browses every past report in `/four`. Beta grants 7 of these across 7 days (whichever first); Pro/Agentic get one a day ongoing. The old one-company / daily-*brief* cron model is **retired** — see `docs/daily-intelligence.md`. Never put the allowlist or customer PII in this public repo.
...
**The exceptions to "static" — the `api/` serverless functions:** `api/advise.js` powers **GrowthKit Live** (the gated tool on `/four`) through a seven-call, dependency-aware first-report pipeline. Research searches the web once and stays internal; six smaller calls generate independently persisted public sections, with 52-second server deadlines and section-only retries. Billing functions power paid Pro; `api/advise.js` + `api/account.js` enforce one full report per UTC day on any company and keep a browsable report history (the daily-brief cron is retired); integration APIs provide encrypted read-only Stripe Connect, GA4, and LinkedIn connections. Server secrets live only in the Vercel dashboard and product APIs fail closed when required auth/storage settings are absent. Full detail: `docs/advisor.md`, `docs/billing.md`, `docs/daily-intelligence.md`, and `docs/integrations.md`.
...
| [`docs/advisor.md`](docs/advisor.md) | The live product: the gated tool on `/four`, seven-call report pipeline, Claude stage prompts/schemas, one-time web research, persistence/retry behavior, `ANTHROPIC_API_KEY`, rate limiting, cost |
...
| [`docs/daily-intelligence.md`](docs/daily-intelligence.md) | The daily-report model: one full report per UTC day on any company, the one-a-day limit, report history, beta report counter, and the retired daily-brief cron |
