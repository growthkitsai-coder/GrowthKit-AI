...
- `contact.html` — short page: hero with a `hero-meta` LED row (manifesto pattern) → two channel cards (email + LinkedIn) with ghost numerals → meta strip. Dark mode is full manifesto-grade console (channel-online LEDs, blinking cursor on the phosphor address lines, neon wireframe ghosts, scan-wash hovers).
...
- `privacy.html` — UK/EEA-aware policy. **GrowthKit AI is the data controller.** Numbered sections (Who we are, What this policy covers, Information collected, Use, Legal basis (UK/EEA), Sharing, Cookies & analytics, International transfers, Retention, Security, Rights, Children, Third-party links, Disclaimers, Liability, Indemnification, Changes, Governing law, Contact). Dark mode is full manifesto-grade console (hero document register with pulsing LED + blinking version cursor, neon section-eyebrow dashes, log-entry scan-wash hovers, terminal-card TOC) — all dark-only pseudo-elements, light HTML untouched.
- `terms.html` — numbered sections covering the agreement, definitions, eligibility, waitlist/pilots/beta, acceptable use, accounts, IP, fees, third parties, confidentiality, liability, termination, governing law. Dark mode is full manifesto-grade console (giant neon-wireframe `§` ghost behind the hero — one dark-only `aria-hidden` span, hidden in light; blinking caret on the version stamp and the TOC label; CRT sweep over the terminal-card TOC; log-entry scan-wash on section hover).
...
- **`.github/workflows/site-checks.yml`** — runs the checker on every push/PR, plus a **lychee** job that checks external URLs only (jsDelivr pins, Google Fonts, LinkedIn, Apps Script). It accepts `999` (LinkedIn bot-block) and `429`, excludes `growthkitai.com` self-references (a new page's canonical would 404 until the same push finishes deploying) and the `fonts.gstatic.com` preconnect root. A weekly Monday cron catches external links that die between pushes. **`--base-url 'https://growthkitai.com/'` is load-bearing:** lychee v0.23 errors on root-relative links (`/waitlist`, `/#engine`) in local files unless they can be resolved; base-url resolves them to growthkitai.com URLs, which the exclude rule then skips (the consistency job owns internal links). Don't remove it or swap it for `--root-dir` — root-dir resolves against the filesystem, where clean URLs don't exist as files, and every page fails again.
