...
- **Pricing (updated 2026-07-22): exactly three tiers.** **Free $0** saves onboarding and shows a locked dashboard preview + public specimen, but cannot execute a deliverable, receive daily intelligence, or use data connections. **Pro $30/month** unlocks the full deliverable, dashboard, daily intelligence, and read-only connections. **Agentic $200/month** adds continuous monitoring, mid-cycle alerts, plan re-cuts, and priority operator review. Beta is not a fourth tier: allowlisted beta users receive temporary Pro-equivalent access. 1-to-1 partner work is direct-contact only. **`pricing.html` is the public source of truth** (Product+Offer JSON-LD in its head); the index FAQ repeats the numbers and links to `/pricing` — keep all three in sync when prices change. Stripe uses distinct Pro and Agentic recurring prices; see `docs/billing.md`. The engine is **un-paused**; pause via `GK_ADVISOR_DISABLED=1` if needed.
- **Product access (updated 2026-07-24):** everyone may create a Free account and save onboarding. Only Pro, Agentic, or current beta grants may execute the deliverable, receive daily briefs, or use integrations. Beta matching trims/lowercases exact email addresses, accepts common Vercel list formats (comma/newline/semicolon or JSON string array), and considers the top-level Supabase email plus verified OAuth identity email locations; user-editable metadata is never trusted. It may be globally time-limited with `GK_BETA_EXPIRES_AT`; `GK_BETA_ENABLED=0` halts beta immediately. If paid/beta access ends, the completed report remains readable, but daily intelligence and integrations lock. Every eligible account gets one company and one full report, then ongoing daily briefs in `/four` on a UTC/GMT day boundary. Never put the allowlist or customer PII in this public repo.
