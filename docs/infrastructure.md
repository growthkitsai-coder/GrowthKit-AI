...
- **Analytics:** Vercel Web Analytics on every page via `/_vercel/insights/script.js`, preceded by a `window.va` queue shim + click listener emitting **custom events**: `cta_click` (clicks on `/waitlist` links and `.nav-cta`/`.btn-primary`, with page + section + href), `waitlist_signup` / `waitlist_error`, `onboarding_submitted` / `onboarding_error`, `status_check_failed`. **Vercel records custom events on Pro/Enterprise only** — on Hobby they're silently ignored (pageviews still work), so don't conclude the wiring is broken if no events show up.

## vercel.json

- **Clean URLs:** a `rewrites` entry (`/privacy` → `/privacy.html`) AND a 301 `redirects` entry (`/privacy.html` → `/privacy`) per page. Current list: `/privacy /waitlist /contact /careers /methodology /terms /manifesto /security /status /specimen /pricing /onboarding` (onboarding is noindex and deliberately out of the sitemap; the checker's `NO_SITEMAP` list knows).
