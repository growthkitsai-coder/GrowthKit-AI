...
- **Server secret:** `ANTHROPIC_API_KEY` — a Vercel env var (Production + Preview), read only by `api/advise.js`. **Never in git** (the repo is public). Set it in the Vercel dashboard **on the original project** (see the two-accounts warning above — as of 2026-07-03 it sits on the dead duplicate and production 503s); see [`docs/advisor.md`](advisor.md). Until it's set there, `/advisor` returns "not configured".

## vercel.json

- **Clean URLs:** a `rewrites` entry (`/privacy` → `/privacy.html`) AND a 301 `redirects` entry (`/privacy.html` → `/privacy`) per page. Current list: `/privacy /waitlist /contact /careers /methodology /terms /manifesto /security /status /specimen /pricing /onboarding /advisor /login /signup /reset /four` (onboarding + the four auth pages `login/signup/reset/four` are noindex and deliberately out of the sitemap; the checker's `NO_SITEMAP` list knows). Auth pages: see [`docs/auth.md`](auth.md).
