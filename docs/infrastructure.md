...
- **Server env vars** (Vercel, never in git — repo is public): `ANTHROPIC_API_KEY` (required — the Claude key read by `api/advise.js`); plus **optional** `SUPABASE_URL` + `SUPABASE_ANON_KEY` which, when set, make `api/advise.js` require a valid Supabase session token (so the now-gated tool can't be run anonymously). **Set them on the ORIGINAL project that serves growthkitai.com** — see the two-accounts warning above; as of 2026-07-03 the Anthropic key sat on the dead duplicate and production 503'd. Auth's client keys live in `auth-config.js` (the anon key is public-safe), not env vars. See [`docs/advisor.md`](advisor.md) + [`docs/auth.md`](auth.md).
