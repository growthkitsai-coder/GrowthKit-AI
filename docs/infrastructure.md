...
- **⚠ Production source drift (confirmed 2026-07-26):** a cache-busted fetch from `https://www.growthkitai.com/product.js` served a manual-application beta flow (`/api/beta`, `beta-approved`, seven reports/one week), while the checked-out and GitHub-tracked `main` at `9c02944` uses the private `GK_BETA_EMAILS` Pro-equivalent allowlist (`beta-allowlist`). This is not browser/CDN staleness and means the domain-owning Vercel project is serving a different source state from `origin/main`; changing or redeploying the allowlist cannot fix that alternate gate. Before debugging entitlement values, confirm the production deployment's Git commit/source repository and production branch in the **original team project**, then deploy the intended `main` source to that project. Recheck a cache-busted `product.js` for `beta-allowlist` and validate authenticated `/api/account` before considering the incident resolved.
