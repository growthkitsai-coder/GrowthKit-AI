...
- **`advisor.css` shared layer:** the GrowthKit Live tool's components (`gk-`-prefixed: console shell, form fields, presets, the designed deliverable — positioning panel / gap cards / play cards / action bar) in both light and dark, using the same tokens. Loaded only by the two pages that host the tool (`advisor.html`, `index.html`). The companion logic is `advisor.js` (see [`docs/advisor.md`](advisor.md)). This is the only component CSS/JS shared beyond `theme.*` — justified by the same tool living on two pages.
