...
- **`advisor.css` shared layer:** the GrowthKit Live tool's components (`gk-`-prefixed: console shell, form fields, presets, the designed deliverable — positioning panel / gap cards / play cards / action bar) in both light and dark, using the same tokens. **Since 2026-07-05 loaded only by `four.html`** (the tool moved behind login; `advisor.html` was deleted and the homepage embed removed). Companion logic: `advisor.js` (see [`docs/advisor.md`](advisor.md) + [`docs/auth.md`](auth.md)). Also note `auth.css` + `auth.js` are shared across the auth pages. These are the only component CSS/JS shared beyond `theme.*`.
- **`threads.js` (index only, 2026-07-05):** the dark hero's WebGL "threads" — flowing neon lines behind the headline, a zero-dependency vanilla port of React Bits' `<Threads />` (the repo is no-React/no-build, so the ogl wrapper was replaced with ~raw WebGL; the fragment shader is verbatim). Mounts into `[data-gk-threads]` inside `.hero`; page CSS hides the mount in light and layers it at `z-index:0`, `opacity:.42`, with a vertical mask fade. **Dark-only and reduced-motion-off by construction:** the script only creates the canvas when `data-theme="dark"` and motion is allowed, tears down on theme toggle (MutationObserver) or `prefers-reduced-motion` change, pauses off-screen/hidden-tab, caps render resolution at 1920, and bails silently if WebGL is unavailable (pure decor). Mouse-reactive (listener on the hero section — the mount is `pointer-events:none`). Config knobs are constants at the top of the file (`COLOR` = `--neon`, `AMPLITUDE` 0.8, subtlety mostly via the CSS opacity).
- **Pioneer-page duplication warning:** `manifesto.html`, `security.html`, `status.html` carry their own inline copies of the tokens + decor CSS (identical values to theme.css — they predate the shared layer), and **`index.html` carries a deliberate fallback copy of the five neon/deep tokens + the dark body background** (added 2026-07-05 after a stale/failed theme.css on a live tab killed every `var(--neon)` on the engine cards while the page's literal `rgba()` rules kept applying — a confusing half-rendered dark mode). Harmless, but **if neon tokens or the dark body background ever change, change theme.css AND those four pages.** Several pages also define local `dkPulse`/`dkBlink` copies identical to theme.css — harmless.
