...
2. **Private beta allowlist** — an exact normalized verified email in `GK_BETA_EMAILS` receives unrestricted Pro-equivalent access.
3. **An approved beta grant** — an approved row in `beta_applications` still inside its 7-day / 7-report window. Keyed on the Supabase **user id**. Full rules: [`beta.md`](beta.md).
...
- **`window.GKBilling` exposes `{ checkout, portal, wire }`.** `wire` was **not exported until 2026-07-27**, and that was a live bug: `init()` calls `wire(document)` once at load, so any `[data-gk-checkout]` element rendered *after* that — the `[data-beta]` card in `product.js` is the real case — was never bound. `product.js` had always called `window.GKBilling.wire(host)`, but an `&&` guard swallowed the missing method silently, so **the "Upgrade to Pro" button shown to an expired beta tester on `/four` did nothing when clicked.** Re-calling `wire` is safe: it stamps each element with `__gkw` and skips anything already bound. **Any code that injects a checkout button after page load must call `GKBilling.wire(container)`** — or call `GKBilling.checkout('pro')` directly, which is what `beta.html` and `advisor.js` do.
