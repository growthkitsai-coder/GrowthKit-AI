...
- **`auth.css`** — shared card/form styling + the `/four` dashboard, widened GrowthKit Live workspace, UTC report countdown, finding checklists, integration, and legacy read-history styling (light + dark). Below 680px the header keeps the status dot/sign-out/theme controls and hides the account email label so the bar cannot overflow.
...
- **`product.js`** — loads `/api/account`, switches `/four` between Free locked preview/onboarding and entitled daily-report states, renders report history plus the next-UTC-unlock countdown, and manages Stripe/Google Analytics/LinkedIn connections. See [`daily-intelligence.md`](daily-intelligence.md) and [`integrations.md`](integrations.md).
- The `/four` product-status line surfaces safe beta state from `/api/account`. An exact verified email in `GK_BETA_EMAILS` reports `beta-allowlist`, shows “Beta access · Pro included”, and bypasses the application card. Other accounts may apply from that card; database states include `beta-pending`, `beta-approved`, `beta-expired`, `beta-reports-spent`, `beta-revoked`, and `beta-disabled`. See [`beta.md`](beta.md).
...
- **`?next=` (added 2026-07-27, for `/beta`)** — `/login?next=%2Fbeta` and `/signup?next=%2Fbeta` return the user to that path after an email/password sign-in instead of dropping them on `/four`. `safeNext()` accepts **only same-origin root-relative paths**: a leading `/` is not sufficient on its own, since `//evil.com` is protocol-relative and `/\evil.com` is normalised to the same thing by several browsers — both are rejected, as is anything over 512 chars, falling back to `REDIRECT_AFTER_LOGIN`. **OAuth `redirectTo` and sign-up `emailRedirectTo` deliberately ignore `next`** and use the configured `HOME`, because those URLs must be registered in Supabase's redirect allowlist (Authentication → URL Configuration); routing them through an arbitrary path would break Google sign-in the moment the path isn't listed. Consequence: an OAuth sign-in started from `/beta` lands on `/four`, whose beta card links back.
...
5. [`202607250001_daily_reports.sql`](../supabase/migrations/202607250001_daily_reports.sql) → daily `reports` + `report_sections` keyed by `report_id`
6. [`202607260001_report_expansion.sql`](../supabase/migrations/202607260001_report_expansion.sql) → ten-stage section constraint

The table-creation migrations are idempotent; the final expansion migration only replaces a check constraint and is non-destructive. **Symptom when skipped (hit for real on 2026-07-24):** `getWorkspace()` gets PostgREST `PGRST205 "Could not find the table 'public.product_workspaces'"`, so `GET /api/account` returns **503 "Your workspace is not available yet"**, and `/four` shows **"We could not confirm your plan. Please try again."** It looks exactly like an Anthropic/API-key outage but no model call is ever made. **Verify quickly** (service-role key, never commit it):
...
- **`/four` onboarding (redesigned 2026-07-08; entitlement gate 2026-07-22):** the signed-in user goes through a **13-step adaptive wizard** covering the full ~25-question startup profile, then review. Before any model call, `advisor.js` awaits the profile upsert and `/api/account` entitlement check. Free users get the saved paywall/specimen state; entitled users enter the ten-stage engine. The saved `_onboarding_complete` marker returns upgraded users directly to review. Fast-track is subject to the same save-before-check rule. `advisor.js` renders from `STEPS`, serializes `profile_text`, and stores one JSON profile row per user.
- **`/four` (gated):** `getSession()` redirects signed-out visitors to `/login`. Signed-in users get server-authoritative access/report state from `/api/account`; no-access accounts see the Pro upgrade, eligible accounts can generate today's report, and completed accounts see report history, the UTC unlock countdown, and connected-data controls. Full-report gaps remain working documents: generated and custom tasks persist independently from the report JSON, and every finding offers a prefilled email request for a relevant founder introduction.
...
- **Full reports** are stored authoritatively in `reports`, one completed report per UTC day; `product_workspaces` and `reads` remain compatibility data for the retired one-company flow. `/api/advise` and all product APIs require `Authorization: Bearer <supabase access token>`.
- **Remember me** routes the session to `localStorage` (default) vs `sessionStorage`; `flowType:'implicit'`.

## Notes / still open

- `/four` is the dashboard: saved onboarding, one full report per UTC day, report history, billing state, integration setup, and legacy reads. Route visibility is client-side, but every paid/beta capability is enforced by server APIs.
