...
async function listReportSections(userId) {
  const r = await adminRequest('/rest/v1/report_sections?user_id=eq.' + encodeURIComponent(userId) + '&select=section,status,output,error,started_at,completed_at,updated_at');
  return r.ok && Array.isArray(r.data) ? r.data : [];
}

async function reserveReportSection(userId, section) {
  const path = '/rest/v1/report_sections?user_id=eq.' + encodeURIComponent(userId) + '&section=eq.' + encodeURIComponent(section) + '&select=*';
  ...
  const now = new Date().toISOString();
  if (existing) {
    const r = await adminRequest('/rest/v1/report_sections?user_id=eq.' + encodeURIComponent(userId) + '&section=eq.' + encodeURIComponent(section), {
  ...
  const r = await adminRequest('/rest/v1/report_sections', {
    ...
    body: JSON.stringify([{ user_id: userId, section, status: 'generating', started_at: now, updated_at: now }])
  });
  if (r.ok && r.data && r.data[0]) return { ok: true, row: r.data[0] };
  if (r.status === 409) return reserveReportSection(userId, section);
  return { ok: false, code: 'unavailable' };
}

async function completeReportSection(userId, section, output) {
  const now = new Date().toISOString();
  const r = await adminRequest('/rest/v1/report_sections?user_id=eq.' + encodeURIComponent(userId) + '&section=eq.' + encodeURIComponent(section) + '&status=eq.generating', {
    method: 'PATCH', headers: { prefer: 'return=representation' },
...
async function failReportSection(userId, section, message) {
  const r = await adminRequest('/rest/v1/report_sections?user_id=eq.' + encodeURIComponent(userId) + '&section=eq.' + encodeURIComponent(section), {
