...
// ── Full reports (Phase 3, 2026-07-27) ──────────────────────────────────────
// A report is one row in `reports`; its ten pipeline sections live in
// `report_sections` keyed by report_id. The full report is the main deliverable
// and is capped at FULL_REPORT_LIMIT completed reports per rolling
// FULL_REPORT_WINDOW_DAYS. The *short* daily update (daily_briefs) is the
// once-a-day habit and is metered separately. See docs/daily-intelligence.md.

const FULL_REPORT_LIMIT = 2;
const FULL_REPORT_WINDOW_DAYS = 7;
const WINDOW_MS = FULL_REPORT_WINDOW_DAYS * 24 * 60 * 60 * 1000;
...
/**
 * Every report row started inside the rolling window, newest first. Filtering on
 * created_at (not completed_at) keeps it to one query; a report goes
 * generating → completed within minutes, so the two are interchangeable for
 * window arithmetic.
 */
async function listReportsInWindow(userId) {
  const r = await adminRequest('/rest/v1/reports?user_id=eq.' + encodeURIComponent(userId) +
...
function finishedAt(row) {
  return new Date(row.completed_at || row.created_at || row.started_at).getTime();
}

/**
 * How many full reports are left in the rolling window, and when the next slot
 * opens. With the limit spent, the next slot opens when the oldest of the
 * newest FULL_REPORT_LIMIT completed reports falls out of the window.
 */
function allowanceFrom(rows) {
  const completed = (rows || [])
    .filter(function (row) { return row.status === 'completed'; })
    .sort(function (a, b) { return finishedAt(b) - finishedAt(a); });
  const remaining = Math.max(0, FULL_REPORT_LIMIT - completed.length);
  let nextAvailableAt = null;
  if (remaining === 0 && completed[FULL_REPORT_LIMIT - 1]) {
    nextAvailableAt = new Date(finishedAt(completed[FULL_REPORT_LIMIT - 1]) + WINDOW_MS).toISOString();
  }
  return {
    used: completed.length,
    limit: FULL_REPORT_LIMIT,
    remaining: remaining,
    window_days: FULL_REPORT_WINDOW_DAYS,
    next_available_at: nextAvailableAt
  };
}

/** The full-report allowance for a user right now. */
async function getReportAllowance(userId) {
  return allowanceFrom(await listReportsInWindow(userId));
}

/** The still-generating report, if one exists (for stage resume). */
async function getActiveReport(userId) {
  const rows = await listReportsInWindow(userId);
  return rows.find(function (row) { return row.status === 'generating'; }) || null;
}

/**
 * The founder's most recent completed report — the company /four follows and the
 * baseline the daily update is cut against. Full row, including full_report.
 */
async function getLatestCompletedReport(userId) {
  const r = await adminRequest('/rest/v1/reports?user_id=eq.' + encodeURIComponent(userId) +
    '&status=eq.completed&select=*&order=completed_at.desc&limit=1');
...
/**
 * Reserve a full report before spending model credits. Enforces the rolling
 * 2-per-7-days cap and resumes an in-progress report rather than duplicating it.
 *
 *   generating & recent          → { ok:true,  code:'resumed'  } (same report)
 *   generating & stale, same co  → reset that row and reuse it   ('reserved')
 *   generating & stale, new co   → abandon it, open a fresh row  ('reserved')
 *   limit spent in the window    → { ok:false, code:'weekly_limit', allowance }
 *   otherwise                    → new report row                ('reserved')
 *
 * Resuming is checked before the cap: an unfinished report has not been charged
 * against the window yet, so it must always be completable. A failed attempt
 * never consumes an allowance slot — only a completed one does — so retries are
 * always allowed. The beta report counter is charged only on completion.
 */
async function reserveReport(userId, input) {
  const rows = await listReportsInWindow(userId);
  const wanted = normalizeCompany(input.company);
  ...
  if (generating) {
    ...
    if (wanted && generating.company_key !== wanted) {
      // A different company: the stale row's saved sections describe the old
      // one, so abandon it rather than reusing report_id and its research pack.
      await failReport(generating.id);
    } else {
      // Same company — reset the row so its sections (report_id) stay valid.
      const now = new Date().toISOString();
  ...
  const allowance = allowanceFrom(rows);
  if (allowance.remaining <= 0) return { ok: false, code: 'weekly_limit', allowance: allowance };
...
/**
 * PostgREST's own reason for refusing a write, for diagnostics. A missing column
 * (an unrun migration) reports here as `42703 column ... does not exist`, which
 * is otherwise invisible behind a generic "unavailable".
 */
function postgrestError(result) {
  const data = result && result.data;
  if (!data || typeof data !== 'object') return null;
  const parts = [data.code, data.message, data.hint].filter(Boolean);
...
async function listDailyBriefs(userId, limit) {
  const r = await adminRequest('/rest/v1/daily_briefs?user_id=eq.' + encodeURIComponent(userId) + '&select=id,brief_date,status,brief,report_id,company_name,created_at&order=brief_date.desc&limit=' + String(limit || 14));
  return r.ok && Array.isArray(r.data) ? r.data : [];
}

/**
 * Reserve today's daily update. `context` carries the report the update is cut
 * against so history stays attributable after the founder switches company; it
 * is written on reservation so even a failed attempt records what it was about.
 */
async function reserveDailyBrief(userId, date, context) {
  const ctx = context || {};
  const stamp = {
    report_id: ctx.report_id || null,
    company_name: ctx.company_name || null,
    company_key: ctx.company_name ? normalizeCompany(ctx.company_name) : null
  };
  ...
  if (existing) {
    const r = await adminRequest('/rest/v1/daily_briefs?id=eq.' + encodeURIComponent(existing.id) + '&updated_at=eq.' + encodeURIComponent(existing.updated_at || existing.created_at), {
      method: 'PATCH',
      headers: { prefer: 'return=representation' },
      body: JSON.stringify(Object.assign({ status: 'generating', error: null, updated_at: now }, stamp))
    });
    if (r.ok && r.data && r.data[0]) return { ok: true, code: 'reserved', brief: r.data[0] };
    return { ok: false, code: 'generating', detail: postgrestError(r) };
  }
  const r = await adminRequest('/rest/v1/daily_briefs', {
    method: 'POST',
    headers: { prefer: 'return=representation' },
    body: JSON.stringify([Object.assign({ user_id: userId, brief_date: date, status: 'generating', updated_at: now }, stamp)])
  });
  if (r.ok && r.data && r.data[0]) return { ok: true, code: 'reserved', brief: r.data[0] };
  return { ok: false, code: r.status === 409 ? 'generating' : 'unavailable', detail: postgrestError(r) };
}
