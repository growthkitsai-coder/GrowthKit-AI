...
/**
 * Supabase normally exposes `user.email`, but some OAuth identities retain the
 * usable address in provider metadata. Check those documented user-object
 * locations too, without relaxing matching to names, ids, or unverified aliases.
 */
 ...
 *      GK_BETA_EMAILS receive Pro-equivalent access. Common Vercel paste
 *      formats are accepted and Supabase OAuth identity emails are considered.
 *
...
async function checkAccess(user) {
  ...
  if (betaCurrent && emails.some(function (email) { return allowlist.indexOf(email) !== -1; })) {
  ...
  if (!betaEnabled) reason = 'beta-disabled';
  else if (!betaCurrent) reason = 'beta-expired';
  else if (allowlist.length) reason = 'beta-email-mismatch';
