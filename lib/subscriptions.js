...
/**
 * Accept the common formats used when pasting private values into Vercel:
 * comma-, semicolon-, or newline-separated text, and simple JSON arrays.
 * Values are normalized in memory only; the private list is never logged.
 */
function betaAllowlist(value) {
  ...
  return Array.from(new Set(entries.map(normalizeEmail).filter(Boolean)));
}

function userEmails(user) {
  const candidates = [user && user.email];
  const identities = user && Array.isArray(user.identities) ? user.identities : [];
  identities.forEach(function (identity) {
    candidates.push(identity && identity.identity_data && identity.identity_data.email);
  });
  return Array.from(new Set(candidates.map(normalizeEmail).filter(Boolean)));
}
 ...
 *   2. Private beta allowlist        — an exact normalized verified email in
 *      GK_BETA_EMAILS receives unrestricted Pro-equivalent access.
 *   3. Approved beta grant           — a row in `beta_applications` that Avi has
 *      approved and that is still inside its 7-day / 7-report window.
 *
 * GK_BETA_ENABLED=0 is a global kill switch, and GK_BETA_EXPIRES_AT is a global
 * cutoff for both allowlisted and database-approved beta access.
 *
 * Database grants fail closed if Supabase is unreachable or the table is missing;
 * the private allowlist does not depend on that table.
 *
...
async function checkAccess(user) {
  ...
  const allowlist = betaAllowlist(process.env.GK_BETA_EMAILS);
  const emails = userEmails(user);
  if (emails.some(function (email) { return allowlist.indexOf(email) !== -1; })) {
    return {
      allowed: true,
      plan: 'pro',
      status: 'beta_allowlist',
      reason: 'beta-allowlist',
      expires_at: betaExpiry || null,
      beta: null
    };
  }

  // Open beta (Avi's decision 2026-07-26): with GK_BETA_OPEN=1, ANY signed-in
  // account may generate — no hand-approval, no application. The one-report-per-
  // UTC-day limit (reserveReport) still caps usage and cost. Flip the env var off
  // to fall back to approval-only; GK_BETA_ENABLED=0 still kills all beta at once.
  if (process.env.GK_BETA_OPEN === '1' && userId) {
    return {
      allowed: true,
      plan: 'pro',
      status: 'beta_open',
