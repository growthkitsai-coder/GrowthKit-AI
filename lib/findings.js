...
const { adminRequest, getReportById, getLatestCompletedReport, getDailyBrief } = require('./product');
...
/**
 * Resolve the report or brief a checklist belongs to.
 *
 * `full_report` reads the `reports` table. It used to read `product_workspaces`,
 * which nothing has populated since the 2026-07-25 report-model change — so
 * every full-report checklist 404'd with `source_not_found` from then until
 * 2026-07-27. `source_id` is now the report's own id, which also gives each
 * report its own checklist state (the old key was per-account, so under the
 * multi-report model every report would have shared one set of tasks).
 */
async function resolveSource(userId, scope, date, reportId) {
  if (scope === 'full_report') {
    const report = reportId
      ? await getReportById(userId, reportId)
      : await getLatestCompletedReport(userId);
    if (!report || report.status !== 'completed' || !report.full_report) return null;
    return {
      source_type: 'full_report',
      source_id: String(report.id),
      findings: extractFindings('full_report', report.full_report)
    };
  }
...
async function loadTasks(userId, scope, date, reportId) {
  const source = await resolveSource(userId, scope, date, reportId);
...
async function createCustomTask(userId, scope, date, findingKey, rawLabel, reportId) {
  const source = await resolveSource(userId, scope, date, reportId);
