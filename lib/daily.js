...
async function generateDailyBrief(user, report) {
  ...
  try {
    ...
    if (!parsed) {
      throw new Error('The daily update did not return readable JSON' + (text ? '' : ' (the model returned no text at all)') + '. Try it again.');
    }
...
module.exports = { generateDailyBrief, extractJson, validBrief, normalizeBrief };
