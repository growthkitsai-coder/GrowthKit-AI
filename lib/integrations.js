...
async function stripeMetrics(row) {
  ...
  return {
    ...
    signups_7d: baselineCustomers.length,
    signups_7d_daily_average: Number((baselineCustomers.length / 7).toFixed(1)),
    revenue_minor: revenue,
    revenue_7d_minor: baselineCharges.reduce(function (sum, x) {
      return sum + Math.max(0, Number(x.amount || 0) - Number(x.amount_refunded || 0));
    }, 0),
    revenue_7d_daily_average_minor: Math.round(sevenDayRevenue),
    churned_subscriptions: yesterdayChurn.length,
    churned_subscriptions_7d: baselineChurn.length,
...
async function googleAnalyticsMetrics(row) {
  ...
  const weekTotal = await gaRun(token, property, '8daysAgo', '2daysAgo');
  const weekAverage = Object.assign({}, weekTotal);
  Object.keys(weekAverage).forEach(function (key) {
    if (typeof weekAverage[key] === 'number') weekAverage[key] = Number((weekAverage[key] / 7).toFixed(1));
  });
  return { yesterday: yesterday, seven_day_total: weekTotal, seven_day_daily_average: weekAverage };
}
...
/**
 * Whether this deployment has the OAuth credentials for a provider at all.
 * `/four` hides unconfigured providers, so nobody clicks Connect and lands on a
 * 503 — LinkedIn in particular ships before its scopes are approved. An already
 * connected provider always reports true so an existing connection stays
 * manageable (and disconnectable) even if a credential is later removed.
 */
function providerConfigured(provider) {
  if (provider === 'stripe') return Boolean(process.env.STRIPE_CONNECT_CLIENT_ID && process.env.STRIPE_SECRET_KEY);
  if (provider === 'google_analytics') return Boolean(process.env.GOOGLE_ANALYTICS_CLIENT_ID && process.env.GOOGLE_ANALYTICS_CLIENT_SECRET);
...
function publicConnections(rows) {
  ...
  return PROVIDERS.map(function (provider) {
    ...
    entry.configured = entry.connected || providerConfigured(provider);
    return entry;
  });
}
