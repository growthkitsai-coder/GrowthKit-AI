...
- `vercel.json` — clean-URL rewrites + 301 redirects + cache headers (images/fonts 1y immutable; **css/js `max-age=0, must-revalidate`** — `theme.css`/`theme.js` aren't fingerprinted and the previous 1h+SWR-1d rule kept serving day-old CSS after deploys, which burned a whole debugging session on 2026-06-11; ETag 304s make revalidation nearly free, so don't "optimize" this back) + security headers (HSTS, X-Content-Type-Options, Referrer-Policy, Permissions-Policy). Current clean-URL list: `/privacy /waitlist /contact /careers /methodology /terms /manifesto /security /status /specimen /pricing` + `/onboarding` (noindex). Note: `security.html`'s terminal readout quotes the security headers verbatim — update it if those change.
